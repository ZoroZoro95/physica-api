...
def _infer_engine_case(text: str) -> str | None:
    ...
    if "thrown horizontally" in text or "projected horizontal" in text:
        if "angle" in text:
    ...
    if ("velocity after" in text or "velocity at" in text or "speed after" in text or "speed at" in text) and "inclined" not in text:
    ...
    if ("thrown horizontally" in text or "projected horizontally" in text) and ("cliff" in text or "tower" in text or "from a height" in text):
    ...
    if "range" in text and "inclined" not in text and "incline" not in text and "tower" not in text and "height" not in text and "cliff" not in text:
...
def _is_horizontal_launch(text: str) -> bool:
    return "thrown horizontally" in text or "projected horizontally" in text or "projected horizontal" in text
...
def _infer_staircase_dimensions(text: str) -> dict[str, str]:
    compact = " ".join(text.split())
    patterns = [
        r"each step is\s*([0-9]+(?:\.[0-9]+)?)\s*m(?:eter|etre|eters|etres)?\s*high\s*and\s*([0-9]+(?:\.[0-9]+)?)\s*m(?:eter|etre|eters|etres)?\s*wide",
        r"each step is\s*y\s*=\s*([0-9]+(?:\.[0-9]+)?)\s*m(?:eter|etre|eters|etres)?\s*high\s*and\s*x\s*=\s*([0-9]+(?:\.[0-9]+)?)\s*m(?:eter|etre|eters|etres)?\s*wide",
